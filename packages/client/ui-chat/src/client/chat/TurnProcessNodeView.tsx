...
export const TurnProcessNodeView = memo(function TurnProcessNodeView({
  node, turnProcess, activeLocale, t,
  ...
  const label = running
    ? phrase !== undefined
      ? duration === undefined ? phrase : t('message.turnProcess.deepDivingPhraseFor', { phrase, duration })
  ...
  const announcement = running ? phrase ?? t('chat.deepDiving')
    : reason === 'aborted' ? t('message.stopped')
      : reason === 'error' ? t('message.turnProcess.failed')
