...
function makeHarness(
  ...
  const renderNodeSlot = ((key: string, owner: object, opts?: {
    ...
    const nodeProps = { ...props, ...nodeOwner, activeLocale: () => locale, useTurnData, useDisclosure, __renders: undefined }
...
describe('ChatView', () => {
  ...
  it('hands running calls to a live Tool group', withClock(2_000, () => {
    ...
    // The running Turn-process row draws one playful phrase fixed at mount; the
    // deterministic random pins it to pool[0], with the elapsed clock appended.
    expect(view.getByRole('status').textContent).toBe(deepDivingPool.zh[0]!)
    const toggle = view.getByRole('button', { name: `${deepDivingPool.zh[0]!} · 用时2秒` }) as HTMLButtonElement
    expect(toggle.disabled).toBe(true)
    expect(toggle.getAttribute('aria-expanded')).toBe('true')
    random.mockRestore()
  }))
  ...
  it('the running clock uses turn/start, ignores steering, and stays out of the live region', withClock(126_000, () => {
    const random = vi.spyOn(Math, 'random').mockReturnValue(0)
    const pool = deepDivingPool.zh[0]!
    ...
    const toggle = view.getByRole('button', { name: `${pool} · 用时2分5秒` }) as HTMLButtonElement
    expect(toggle).toBe(turnProcessControl(view.container))
    expect(toggle.disabled).toBe(true)
    expect(status.textContent).toBe(pool)
    ...
    expect(toggle.textContent).toBe(`${pool} · 用时2分5秒`)
    act(() => { vi.advanceTimersByTime(2_000) })
    expect(toggle.textContent).toBe(`${pool} · 用时2分7秒`)
    ...
    act(() => { vi.advanceTimersByTime(1_000) })
    expect(toggle.textContent).toBe(`${pool} · 用时2分8秒`)
    expect(status.textContent).toBe(pool)
    view.unmount()
  ...
  it('the running clock reads hours once the turn passes an hour', withClock(3_600_000, () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const toggle = view.getByRole('button', { name: `${pool} · 用时59分59秒` })
    act(() => { vi.advanceTimersByTime(1_000) })
    expect(toggle.textContent).toBe(`${pool} · 用时1小时00分0秒`)
    act(() => { vi.advanceTimersByTime(303_000) })
    expect(toggle.textContent).toBe(`${pool} · 用时1小时05分3秒`)
    expect(view.getByRole('status').textContent).toBe(pool)
    view.unmount()
    expect(vi.getTimerCount()).toBe(0)
    random.mockRestore()
  }))
