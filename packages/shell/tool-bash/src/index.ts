...
function bashDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a bash command and return its stdout/stderr. '
  ...
  return base + ' Attempting a command the sandbox may deny is safe and expected: run it and read the '
    ...
    + 'succeed, retry the exact command once with `sandbox_permissions` (the narrowest wider mode that suffices) '
    + 'plus a one-sentence `justification`; the approval prompt raised by that retry is how the user consents. '
    + 'Never escalate speculatively: ground the request in a real denial. If the session states approval '
    + 'prompts are disabled, a denial is final — do not set `sandbox_permissions`. '
